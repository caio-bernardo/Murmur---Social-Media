...
class AccountsTest(TestCase):
    ...
    async def test_username_already_inuse(self):
        same = "tester"
        await User.objects.acreate_user(username=same)
        payload = {
            "username": same,
            "email": "tester@email.com",
            "password": "12345678",
    ...
    async def test_get_own_profile(self):
        ...
        res = await self.tclient.get(
            "/me", headers={"Authorization": "Bearer " + str(refresh.access_token)}
        )  # type: ignore
    ...
    async def test_get_public_user(self):
        ...
        user = await User.objects.acreate_user(
            username="public_test_user", first_name="Public", last_name="User"
        )
        # Hit GET /{username}
    ...
    async def test_delete_user_account(self):
        # Create a user and log in
        user = await User.objects.acreate_user(username="deleter", password="123456")
        refresh = RefreshToken.for_user(user)
    ...
    async def test_upload_user_photo(self):
        ...
        user = await User.objects.acreate_user(
            username="photo_tester", password="12345678"
        )
        refresh = RefreshToken.for_user(user)
        ...
        res = await self.tclient.post(
            ...
            FILES={
                "file": SimpleUploadedFile(
                    "test_image.jpg", fake_image.getvalue(), "image/jpeg"
                )
            },
        )  # type: ignore
    ...
    async def test_delete_user_photo(self):
        user = await User.objects.acreate_user(
            username="photo_deleter", password="12345678"
        )
        refresh = RefreshToken.for_user(user)
