...
from comments.apis import router
...
class CommentsTest(TestCase):
    ...
    async def test_get_a_single_comment(self):
        ...
        response = await self.tclient.get(f"/{self.comment1.pk}")  # type: ignore
        self.assertEqual(response.status_code, 200)
    ...
    async def test_delete_comment(self):
        ...
        response = await self.tclient.delete(
            f"/{comment_id}", headers={"Authorization": f"Bearer {self.token_user1}"}
        )  # type: ignore
    ...
    async def test_get_comments_with_pagination(self):
        ...
        response = await self.tclient.get(f"/?post={self.post.pk}&limit=1&offset=0")  # type: ignore
        self.assertEqual(response.status_code, 200)
        # Should return exactly 1 comment
        self.assertEqual(len(response.json()["items"]), 1)

        # Get the next page
        response = await self.tclient.get(f"/?post={self.post.pk}&limit=1&offset=1")  # type: ignore
        self.assertEqual(response.status_code, 200)
        ...
        for offset in range(0, total_comments, 1):
            response = await self.tclient.get(
                f"/?post={self.post.pk}&limit=1&offset={offset}"
            )  # type: ignore
